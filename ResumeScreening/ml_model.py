...
def train_shortlist_model() -> LogisticRegression:
    X = np.array(
        [
            [2, 1, 25],
            [3, 1, 30],
...
def predict_candidate_status(model: LogisticRegression, skills: list[str], matched: list[str], score: float) -> tuple[str, float]:
